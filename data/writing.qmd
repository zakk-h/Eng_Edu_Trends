...
The residual plot shows great symmetry across the residual = 0 line, indicating that the linearity assumption in the parameters is indeed met. Visually, homoscedasticity looks reasonable, but there is more variance when fitted values are 0. Code was used to find 6 quantiles, split the observations into quintiles based on those quantiles, and then calculate the variance of the residuals for each quintile. From there, we quantitatively assessed homoscedasticity, and each decile’s variance was reasonably close to all others, with the exception being where the fitted values are 0. The Breusch-Pagan test finds statistically significant evidence for heteroscedasticity. However, we moved forward with this model because the heteroscedasticity does not bias the estimates for the coefficients. We also explored transformations in the predictors but did not find them worth pursuing. Specifically, we tried Box-Cox transformations.
...
We predicted the average highest education qualification for a city/town achieved that ranged between 2.57 and 5.14  in our dataset. Our predictors were all untransformed: 

Level4qual_residents35_64_2011 (Low, Medium; High as the baseline)

Level_3_at_age_18

Activity_at_age_19_full_time_higher_education

Rgn11nm_combined (North East, North West, Other; South \[South East + South West\] as the baseline 

Key_stage_4_attainment_school_year_2012_to_2013

The interaction between level4qual and level_3.
